...
if [ "x$TMP01" != "x" ] ; then
    ...
    CONFIG=" -c $(dirname "$(realpath "$0")")""/../site/gearshift/local.conf"
elif [ "x$TMPSCRATCH" != "x" ]; then
    DATADIR=$TMPSCRATCH/apps/
    #ESTPREFIX="_habrok"
fi
bash run_project.sh -i tests/integration/input${TESTPREFIX}.json -s ./tests/integration/config/samples.json -r ./tests/runs/local/ -f ./tests/data/fastq -d $DATADIR $CONFIG
